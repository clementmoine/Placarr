...
> **Archivé le 2026-08-24** — Les manques encore ouverts sont versés au backlog, couverts par le programme de réorganisation en cours.
...
### 4. Reprise après coupure — sweep livré, checkpoint encore absent

`BackgroundWorkJob` porte `status`, `attempts`, `updatedAt`. **Le balayage
existe désormais** : le worker tient `updatedAt` à jour par heartbeat
(`FOIL_LOCK_HEARTBEAT_MS`, `src/core/collect/jobs/workRunner.ts`), et
`recoverStaleRunningBackgroundWorkJobs` (`src/core/collect/jobs/workQueue.ts`)
balaie les jobs restés `running` dont le worker a disparu — ils ne « sit
`running` forever » plus. Un conteneur qui redémarre en plein travail ne
laisse donc plus un job mort en base bloquant son `replaceOpenForKind`.
...
**Ce qui reste ouvert, c'est le point de contrôle**, pas la requeue. Nos étapes
sont déjà idempotentes — la passe de faces saute ce qui
