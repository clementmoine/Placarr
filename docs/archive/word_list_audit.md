...
| `GENERIC_TITLE_TOKENS`, `RESOLVER_GENERIC_TOKENS`, `RESOLVER_PLATFORM_TOKENS`, `NON_CANONICAL_CONTEXT_TOKENS` | `evidence/*`, `identityNoise.ts`          | **DRY 2026-07-24**: PriceCharting uses `IDENTITY_FUNCTION_WORDS`; retailer `titleMatch` uses `IDENTITY_EDITION_PACKAGING_TOKENS` + `IDENTITY_PLATFORM_NOISE_TOKENS` (hardware keep-platform). **IDF MVP + offline RawName DF + boot/cron 2026-07-24**. **GENERIC fully derived 2026-07-24**: function words + `IDENTITY_MEDIA_CATEGORY_TOKENS` (`jeu/game*` from `LISTING_NOISE` / lot plurals) + IDENTITY-backed `edition`/`version`. No residual bare domain chrome. |
| ~~`TITLE_STOP_WORDS`~~ **FIXED 2026-07-24**                                                                   | `pricecharting/fetch.ts`                  | Deleted local set → `IDENTITY_FUNCTION_WORDS`                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `BROAD_SCREENSCRAPER_FALLBACK_WORDS`, `NON_DISTINCTIVE_SCREENSCRAPER_TOKENS`                                  | `screenscraper/resolver.ts`               | Composed from `GENERIC_TITLE_TOKENS` / listingTerms + thin SS-local seed policy (`club`/`star`/`super`/`force`). **`jeux` via GENERIC ; `complet`/`complete` via `LISTING_CONDITION_TERMS` (2026-07-24)**.                                                                                                                                                                                                                                                             |
