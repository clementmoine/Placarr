...
| **P1**   | **Structure — réduire l'éparpillement (étude tako-firehouse)** | Mesuré 2026-08-15 : 917 fichiers source (hors 509 tests) contre 215 chez [tako-firehouse](https://github.com/Nimai26/tako-firehouse), périmètre proche. Mais nos fichiers de provider font **253 lignes de moyenne** contre ~300 chez eux, médiane 3 fichiers/provider — l'éparpillement est dans `core/` (487 fichiers, dont `core/enrich` à 142 avec 65 sous 100 lignes), pas dans `providers/`. À reprendre : découpage **par domaine** plutôt que par couche, un fichier par provider, `BaseProvider` explicite. Chiffres, réserves et ordre dans [structure_vs_tako.md](structure_vs_tako.md). |
