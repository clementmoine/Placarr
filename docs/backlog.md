...
| **P2** | Scrape-yield / call efficiency | **PC + gates + evidence + DetailYield + gap-fill + eBay Browse reuse 2026-07-24**: FetchStore ; abort ; scrape+**API** seed gates ; `ProviderEvidence` ; retail/PC reuse ; metadata seed ; eBay shared Browse SearchYield (meta↔prix). Reste : corpora, Flare 1→N, SearchYield durable. Voir [scrape_yield.md](scrape_yield.md). |
