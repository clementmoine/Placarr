...
| **P2** | Scrape-yield / call efficiency | **PC Phase 1 + timeout abort 2026-07-24**: search-first/soft-404/FetchStore ; price job timeout **aborts** Flare/HTTP via `jobAbort` ALS. Reste : évidence durable, cost tiers, eBay batch, corpora locaux. Voir [scrape_yield.md](scrape_yield.md). |
