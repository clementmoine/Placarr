...
| **P4**     | `ProviderInfo.weight` résiduel                  | **Ouvert**          | Deux usages réels restants : `nameDatabaseProviderForType` (tie 3-way sur books — booknode/bedetheque/openlibrary tous à 0.5 → ordre registry arbitraire) et `primaryGameCoverSource` (admin metadata-enrich). Les poids viennent de `PROVIDER_METADATA_EXTENSIONS` (registry). Remplacer par des traits explicites change des sélections de providers non couvertes par les fixtures → à faire avec validation live (même leçon que le modèle de confiance des overrides).                                                  |
