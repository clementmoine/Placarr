...
| **P3** | PicClick → eBay Browse API | **Ouvert (recommandé)** | **Investigation 2026-06-29** : PicClick résout vers une IP AWS us-west (`54.176.32.72`), le TCP connect *hang* (pas de réponse, `http=000`) → chaque scan paie le timeout 6 s puis `[]`. Blocage réseau dur depuis cet env ; en plus le TOS PicClick **interdit le scraping**. PicClick n’est qu’un reskin eBay. Remplacer le scrape HTML par l’**eBay Browse API** (`/buy/browse/v1/item_summary/search`, OAuth `client_credentials`, JSON) — TOS-compliant, fiable, source directe. Source la moins pondérée (`sourceWeight 0.08`) donc P3. |
