...
| **P2** | Scrape-yield / call efficiency | **PC + gates + evidence + DetailYield + gap-fill 2026-07-24**: FetchStore ; abort ; scrape+**API** seed gates ; `ProviderEvidence` ; retail/PC reuse ; metadata force-refresh **seeds** fiche (pas de blank-slate Tier 0+1). Reste : eBay batch, corpora. Voir [scrape_yield.md](scrape_yield.md). |
