...
| **P2** | Scrape-yield / call efficiency | **PC + gate + evidence + DetailYield + price gap-fill 2026-07-24**: FetchStore ; abort ; scrape gate ; `ProviderEvidence` ; factory/boardgame `productUrl` ; retail refresh **reuses** fresh evidence (0 HTTP). Reste : metadata seed gap-fill, eBay batch, corpora. Voir [scrape_yield.md](scrape_yield.md). |
