...
> Dernière vérification : **2026-08-13** (note No-Intro usage ; catalogues locaux).
...
| **P2**   | **TCG — harmoniser le contrat des packs cartes** | Audit mesuré 2026-08-15 : Pokémon n'a ni `prints` ni `searchPrints` (seul manque fonctionnel) ; `print_assets` a **quatre** formes ; `STEPS` déclarées seulement chez dbscg/dbsfw. Cible et ordre dans [card_pack_contract.md](card_pack_contract.md). **Fait** : `<role>.<source>.<ext>` et fin du réencodage lossy des faces. **Ne pas** aplatir les vernis Lorcana, le `wayback_timestamp` Naruto ni le `card_foil` Pokémon — sources réellement différentes. |
| **P3**   | **Check-list de collection + conseil d'achat** | *Fonctionnalité utilisateur*, pas de la dette. **À implémenter après** la consolidation de l'existant et l'ajout de nouvelles sources (nouveaux TCG) — décidé 2026-08-15. Vue par collection listant tout ce qui existe, case cochée si possédé, vignette, export imprimable. **Toute la donnée est déjà locale** : LaunchBox 4 613 jeux PS1 (182 172 tous supports, 1 290 570 images), Naruto 771 tirages, DBS Masters 8 434, Pokémon 93 777. C'est une jointure catalogue × items, aucun réseau. **Prérequis structurel** : il n'existe **aucune table de sets** — `set_name` est une chaîne dénormalisée, absente chez Naruto ; ni date de sortie, ni statut, ni territoire. Or le statut est **par territoire** : la S6 Naruto est annulée en France et sortie en Italie. Un set doit porter code/date/territoire/statut **avec sa preuve**. Et `Shelf` n'a aucun lien vers un catalogue : une check-list s'ancre sur un **catalogue + portée**, jamais sur une étagère. **Un catalogue ne contient pas que des cartes** : decks, coffrets, displays — et `set_name` confond déjà set de boosters et produit scellé (« BT31 » à côté de « DECK DE DÉMARRAGE -Final Radiance- »). Matière déjà là : pages decks Naruto sur disque, taxonomie produits dbscards. **Un produit contient des tirages, et ça traverse les sets** — mesuré : le deck *Final Radiance* contient 19 cartes de **11 sets** d'origine, la *Premium Anniversary Box 2024* 52 cartes de 9 sets. La donnée est déjà là, sous deux noms trompeurs : `set_code` = set d'origine, `set_name` = produit de distribution (même notion que le segment `evp17` des URLs dbscards). Ça débloque la vraie question du collectionneur : non pas « que me manque-t-il » mais **« que dois-je acheter »** — recouvrement d'ensembles pondéré, glouton suffisant à notre échelle. **Deux familles** : contenu déterministe (deck, coffret) → chiffre exact ; contenu aléatoire (booster, display) → **prix de bascule** « achète à l'unité sauf si tu trouves une display sous X € », calculable **sans taux de tirage** : la rareté est déjà renseignée à 100 % (12 318 Lorcana, 15 662 DBS) et seule la composition d'un booster manque. Les prix à l'unité existent déjà (7 092 cartes FR cotées, 99 212 € cumulés) et révèlent une falaise : BT31 coûte 2 404 € à compléter, médiane 0,10 €, max 399 € — viser « le set complet » n'a pas de sens, il faut montrer la marche. Manque : le prix des produits scellés. Forme retenue : **chaque catalogue déclare ses types d'entrée** sur un minimum commun, pas de taxonomie universelle du collectionnable. **Posséder du scellé est une nature, pas une condition** : un booster possédé est scellé par définition — ouvert, ce sont des cartes. `Item.condition` (`new|used|loose|damaged`) ne peut pas le porter. Hiérarchie uniforme `display → N boosters → M cartes`, `deck → K cartes` : **ouvrir = remplacer le contenant par son contenu**, donc « ouvert » n'est pas un état à stocker et un booster déballé cesse d'exister. Booster/display scellé = contenu inconnu ; deck scellé = contenu connu mais pas en main ; d'où **deux complétions défendables** (« cartes détenues » vs « scellé compris »), à ne pas trancher en silence. **Hors format non géré** : `faceQuarterTurns` existe au contrat et sert chez Pokémon/Lorcana, mais DBS ne le renseigne pas alors qu'il a de vraies cartes paysage (BT24-061, 20 faces hors ratio sur 27 739) ; et les cartes jumbo n'ont **aucune** notion de dimension nulle part. **Enregistrer les ouvertures dès le départ** : c'est ce qui produira les taux de tirage que les éditeurs ne publient pas, et c'est irrécupérable après coup. Base existante : `narutoccg/buildCoverageChecklist.ts`, à généraliser en vue. Questions ouvertes et voie PDF dans [collection_checklist.md](collection_checklist.md) — dont la clé de jointure côté jeux, seule vraie inconnue. |
| **P3**   | **Gabarits de cartes / cartes personnalisées (DBS)** | À discuter. Six PNG de cadres par couleur sont dans `src/providers/dbscg/curated/template/` (`{red,blue,green,yellow,dark}-template.png` + `assets.png`, 428 Ko) — versés le 2026-08-15, **aucun code ne les référence**. Référence apportée par l'utilisateur : [custom card template for DBS Masters](https://www.reddit.com/r/DBS_CardGame/comments/1lznuui/custom_card_template_for_dbs_masters_on_card/). Sujet à cadrer : rendu de cartes personnalisées / proxies, et ce que ça implique côté catalogue (une carte custom n'est pas un tirage officiel — ne doit pas compter dans une complétion). |
| **P1**   | **Architecture — chemin de lecture + `BaseProvider`** | Analyse complète tako-firehouse le 2026-08-15 dans [structure_vs_tako.md](structure_vs_tako.md). Diagnostic : ce n'est pas le nombre de fichiers, c'est l'**absence de chemin de lecture**. Leur système tient en 4 fichiers (`content-types`, `BaseProvider`, `BaseNormalizer`, `sources`) ; comprendre `bgg` demande 2 fichiers contre ~20 pour `dbscg`. Notre `providerModule.ts` est **déclaratif** là où leur `BaseProvider` est **exécutable**. Ordre : (1) `BaseProvider` sur `bedetheque` seul, (2) `ARCHITECTURE.md` = les 5 fichiers à lire dans l'ordre, (3) ouvrir `docs/ADR.md`, (4) regrouper `core/enrich/{titles,media}` (92 fichiers). **Garder** : nos 509 tests (eux : 0) et la sophistication de `core/enrich` (23 888 lignes contre 450 — pas la même tâche). |
| **P1**   | **Autonomie du conteneur — finir la couverture** | Audit 2026-08-15 dans [autonomy_audit.md](autonomy_audit.md). Le Plex-like est **déjà construit** : `init.sh` (migrate → workers → Next, conteneur descendu si un worker meurt), `catalogueAutoSync` horaire sur `status().stale`, file persistée avec `attempts`/`maxAttempts=3`, repli FlareSolverr automatique sur 403/429/503 + signatures Cloudflare (29 providers scrapeurs sur 65 — les 36 autres sont des API), cadence déclarée par provider, 429 volontairement non réessayé. **Manques** : aucun job `running` n'est récupéré après une coupure (« they would sit `running` forever ») — politique voulue : reprendre si la coupure est récente, repartir de zéro au-delà de ~24 h, et **noter l'étape atteinte** pour que la reprise saute ce qui est fait ; `softban` persistant sur 3 packs seulement ; **18 endroits en `fetch` nu** qui perdent le signal d'abandon ; `refresh` tout-ou-rien. **Cas observé 2026-08-15** : `tsx watch` a redémarré le worker à chaque édition, empilant six pipelines complets en trois heures avec des enfants orphelins, `bridé` à 563 sans refroidissement. Ordre : garde de test contre `fetch(` nu → reprendre les 18 → généraliser `softban` → granularité. **Cadrage produit** : l'admin est un outil de debug de dernier recours, pas une surface de production ; aucun ajout de provider à chaud (build + déploiement). |
| P2       | TCG — autres jeux            | Pokémon (TCGdex, prix en €), Magic, Yu-Gi-Oh gratuits ; One Piece / DBS **Fusion World** via clé apitcg — [one_piece_tcg.md](one_piece_tcg.md) ; **DBS Masters** livré (`dbscg`, `pnpm dbs:cards`) — [dragon_ball_super_card_game.md](dragon_ball_super_card_game.md) ; Naruto (pack `naruto`, sets `s*` + lang `fr|en|jap` puis autres lignes) — [naruto_carddass_tcg.md](naruto_carddass_tcg.md). |
| **P2**   | **Catalogues locaux finis**  | Modèle : [provider_supply_modes.md](provider_supply_modes.md). Provider `narutoccg` + hook `catalog` ; data vs curated ; Catalogue hub (all/unitaire/auto). CLI / pipeline sous le module. Trous S5 — [naruto_carddass_fr_recovery.md](naruto_carddass_fr_recovery.md). |
| P2       | Regroupement des doublons    | Transform d'affichage : `Elsa foil ×3` si **même** metadata + variante + langue + **Condition à 100 %**. Voir [tcg_support.md](tcg_support.md) §4. |
...
| P3       | Clarifier No-Intro (usage produit) | Provider `nointro` + index SQLite + checksum path **existent**, mais le rôle produit est opaque : quand ça sert (ROM dump hash vs titre), comment obtenir les DAT (DAT-o-MATIC = pas d’URL stable / POST session — ≠ LaunchBox Metadata.zip), quoi configurer (`NOINTRO_DAT_*`), UX admin Catalogue si corpus absent. Doc courte + décider si on garde / on simplifie / on documente « manuel only ». Voir `src/providers/nointro/`, [archive/scrape_yield.md](archive/scrape_yield.md) §3. |
...
| ~~P2~~   | Sync catalogue TCG local (fraîcheur) | **Fait** — sync auto `foilCatalogSync` (staleness) + manuel admin/CLI ; indexes data-only ; gaps admin foil-status (`computeFoilGaps`) ; APK vs réseau : [foil_apk_sources.md](foil_apk_sources.md). |
...
**CLI :** `pnpm foil:pokemon` · `pnpm foil:pokemon:scrape` · `pnpm foil:pokemon:sources` · `pnpm foil:pokemon:index-cards` · audits via `tsx src/providers/pokemontcglive/audit*.ts` · `pnpm foil:lorcana` · `pnpm foil:lorcana:cards`.
...
| 4 | ~~Audit store~~ | `tsx src/providers/pokemontcglive/audit_store.ts` — shaders / masques / `cards.json` |
...
Conséquence pratique : le regroupement des doublons (P2) inclut l'état **et**
la variante dans sa clé — tant que `loose` vit dans `Condition`, un jeu loose
et le même jeu complet restent des groupes distincts (voulu). Le jour où
`loose` migre sur l'axe variante, la clé reste cohérente. À traiter quand
...
- [card_pack_contract.md](card_pack_contract.md)
- [structure_vs_tako.md](structure_vs_tako.md)
- [autonomy_audit.md](autonomy_audit.md)
- [collection_checklist.md](collection_checklist.md)
