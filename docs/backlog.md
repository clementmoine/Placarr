...
| **P3**   | `no-explicit-any` sweep              | **Ouvert**          | ~130 `any` (dette accumulée pendant que lint était cassé) passés en `warn` dans `eslint.config.mjs` ; typer puis repasser la règle en `error`.                                                                                                                                                                                             |
...
| Priorité | Item                                        | État       | Détail / prochaine action                                                                                                                                                                                                                                                                                                                                                |
| -------- | ------------------------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **P2**   | `tokenEquivalents.ts` par-produit           | **Ouvert** | `TITLE_PHRASE_EQUIVALENT_GROUPS` = map FR↔EN par produit (« roi des corrompus »→« taken king », « baphomet »→« broken sword »…), non inventoriée jusqu'ici. Cible : équivalence via **alternate names providers** (`alternateNames.ts`, IGDB/SS) + consensus ; garder seulement les paires génériques (couleurs). Consommé par search/titleMatching/boardGame/retailer. |
| **P2**   | Boosts FR résiduels `title/displayScore.ts` | **Ouvert** | Verdict « DELETE » de `word_list_audit` partiellement exécuté : word-boosts + CJK supprimés, restent accent `+50` / FR `+30` / `+1500`. La préférence de langue doit venir de la locale UI (`languageOrderForUiLocale`). Retrait test-gated + bump `BARCODE_CACHE_VERSION` (titres affichés changent).                                                                   |
| **P4**   | `ProviderInfo.weight` résiduel              | **Ouvert** | Un seul usage restant : tie-break `nameDatabaseProviderForType` (registry.ts). Propriété déclarée (acceptable), mais l'ordre pourrait être un trait explicite.                                                                                                                                                                                                           |
| **P4**   | Bump cache non fait (calibration 06-29/30)  | **Décidé** | Cluster confidence +0.06 et rétention d'observations rejetées ont modifié la logique après le bump v40 **sans** bump. Leaders/platformKeys inchangés → entrées cachées pas « fausses », coût d'invalidation totale non justifié. Règle : prochain changement qui altère titres/leaders ⇒ bump.                                                                           |
