...
| **P2** | Scrape-yield / call efficiency | **PC + gates + evidence + DetailYield + gap-fill + eBay + Presta SearchYield 2026-07-24**: FetchStore ; abort ; scrape+API seed gates ; `ProviderEvidence` ; retail/PC reuse ; metadata seed ; eBay Browse reuse ; Presta barcode enrich SearchYield-first (0/shortlist fiche). Reste : Smartoys 1→1, corpora, SearchYield durable. Voir [scrape_yield.md](scrape_yield.md). |
