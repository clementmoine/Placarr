...
| **P2** | Scrape-yield / call efficiency | **PC + gate + evidence + DetailYield 2026-07-24**: FetchStore/soft-404 ; abort ; scrape gate ; `ProviderEvidence` ; Philibert/… + **Presta/Shopify factory** keep `productUrl`+scan `sourceUrl`. Reste : gap-fill, eBay batch, corpora. Voir [scrape_yield.md](scrape_yield.md). |
