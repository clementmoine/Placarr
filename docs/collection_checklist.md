...
- **DBS / Lorcana / Pokémon papier / One Piece** — même logiciel
  ...
  `src/providers/shared/tcgcards/sites.ts`, un crawl. Accessoires exclus.
...
| ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `data/dragonball/cg/staging/dbscards-products/`          | **224** SKU ; **190** fiches (120 boosters + 70 decks / coffrets / special-packs)                                                   | date, prix, `declaredCardCount`, URL d'image. **2451** liens carte. Decks / coffrets exclusifs complétés depuis `catalog.sqlite`. Un booster reste sur l'aperçu 15 tuiles (Union Force 15/127) — le site le dit ; on ne dump pas le set. |
| `data/lorcana/staging/lorcards-products/`         | **122** SKU ; **110** fiches (boosters / blisters / decks / coffrets / troves / **puzzles**). Displays en index.                                  | date, prix, URL d'image, aperçu 15 tuiles. **1491** liens carte. Un booster Set 12 Woody = 15/446 — le pool du chapitre, pas le contenu du pack. Puzzles Glimmers → kind `puzzle`, promo `lorcana:p2-36` garantie (checklist « Inclus dans »).                                           |
...
- **Pokémon** — TCGdex : nom + logo (`/sets/{id}`, URL `…/logo.png`). Nom
  ...
  → abbr officielle / id TCGdex) via `pack_documents.tcgdex-set-logos`.
