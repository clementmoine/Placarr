...
provider `narutocarddass`. À parcourir un par un pour faces, dos, displays,
...
| **FR** |      41 | S1–S5 named+art **complet**. Reste : S6 jamais imprimée (`cl0033`, ni0257–283, ni0309, ta0240, te0263), stubs `*-promo`. `pr0096` FR = Leboncoin (toploader) ; EN = Drive + Coleka `_r38199`.   |
...
1. **Drive Enhanced → `cards/`** : `Catalogue Sync -- --only scrape --locale drive`
...
| Site officiel italien                                | `carddass.it`, `bandai.it/naruto*`, `narutocardgame.it`, `narutocarddass.it` : **0 capture**. Il n'y a pas d'équivalent italien de `carddass.fr`, qui reste la raison pour laquelle le FR est à 95 % et l'IT à 19 %.                                                                                                                                                  |
...
1. **Promouvoir Drive Enhanced → `cards/`** — **pas fait**. Les 6 zips Google
   ...
   ```sh
   Catalogue Sync -- --only scrape --locale drive
   # optionnel : --sets s1,s2,s28
   # prochaine fois, unzip auto :
   Catalogue Sync -- --only scrape --locale drive --staging-only \
...
2. **Index / layout GAKU SHI MJU MSA** — code disque (`gaku/` `shi/` `mju/`
   ...
   `Catalogue Sync -- --offline --only index` (ou sync admin) a bien
...
2. Ultrajeux Wayback `serie_5/` — 6 JPEG officiels des trous S5. **Done 2026-08-17** : `art.ultrajeux` + `staging/ultrajeux/serie_5/`. `Catalogue Sync -- --locale ultrajeux`.
...
17. narutocarddassfrance — fils restants (tome 12 **nom** clos = Neji Rare, numéro TBD ; 7ᵉ inédite ; présentoir Kana).
...
sont dans le journal archivé. **Dig 2026-08-26** :
`curated/sources/fr-promo-dig-2026-08-26.json` + **chasse UNS3 EU**
`uns3-eu-promos-hunt.json` + `goat-locale-promos.json` + `user-physical-ccg.json`.

**Canal map (ne pas fusionner)** :
| Canal | Promo | Catalogue |
|---|---|---|
| Day One jeu EU | PR-095 (Goku / Unique Training) | titre FR stub `Naruto Uzumaki` ; effet FR TBD ; art FR manquant |
| Duopack retail FR | PR-096 (4e Hokage) | titres+art FR OK ; sealed `duopack-s28` + packshot eBay |
| Goat French Foil | PR-100 (cape Hokage / ASPIRATION) | titre FR stub ; art FR manquant |
| Collectors WoF / True Despair | PR TBD | photos scellées encore |

Tin Carddass reste **PR-011 / PR-016** seulement. TCDB sid 118974 = grab-bag, pas d’ingest.
1. ~~**Tome 12**~~ — **nom clos** dans t1185 (Katagena) : « Neji Hyuta (Rare) ».
   Numéro imprimé encore **TBD** (ne pas inventer l’id).
...
5. **La 2ᵉ tin box** — Amazon dit « le premier coffret » et le forum évoque une
   ...
   françaises » Carddass tin est **tranchée : non** — deux tin PR FR
   attestées. Ne pas confondre avec le set promo **US CCG** `PR-001…`.)
...
| Coleka `_r38199` promos US                                                                                                                                                                                      | `art.coleka` promo EN             | —   | —                             | EN CCG, ≠ tin FR                      | done        | 101 fiches (`PR-001`–`093`/`095`/`096`/`100` + `005R`–`009R`). Listing `scrape: true`. Disque `cards/promo/pr0nnn/en/`. Drive 750×1050 reste l’affichage EN quand il est plus grand. Pas de fiche FR « 4e Hokage » (Leboncoin PR-096). Pas l’ombrelle `_r4102`.                                                                                                                                                 |
...
| [narutocarddassfrance](https://narutocarddassfrance.1fr1.net/)                                                                                                                                                            | scans promo                       | —   | tin 5130                      | shuriken, blister Kana                | recheck     | 332 sujets ; Wayback 1 URL — crawler **live** + sauver. Pièges t1132/t1021.                                                                                                                                                                                                                                                                                                                                     |
| forumnarutocarddass.com                                                                                                                                                                                              | —                                 | —   | —                             | alias                                 | done        | Même board.                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Leboncoin `ad-large`                                                                                                                                                                                            | PR-096 FR `art.leboncoin`         | —   | —                             | toploader 602×800                     | watch       | URL collée (annonce 3233037633). Pas de crawl boutique. Filigrane + sleeve. Coleka n’a pas la VF.                                                                                                                                                                                                                                                                                                               |
...
| [cartes-naruto.com manga-cartes](https://web.archive.org/web/20081217130741/http://www.cartes-naruto.com/manga-cartes.html)                                                                                     | —                                 | —   | —                             | checklist ⊆ names                     | done        | Hub séries 1–5. **#65** = S4 decks 40+40 ; **#78** = sortie S5 packshots only ; **#134** = S6 decks qty. Forum t1057/t1827/t1285 staged — pas de listes S5 usine. `cartes-naruto-com-forum-hunt.json`.                                                                                                                                                                                                                                                                           |
| [joueur.ch S5 starters](https://joueur.ch/trading-cards/naruto-la-quete-serie-5/)                                                                                                                                 | packshots face                    | —   | —                             | sealed retail                         | **lead**    | Quête + Nouveau départ en stock (~CHF 70). Faces Ref. 05124 staged `market-s5/joueur.ch/`. **Demander livret / verso** ou ouvrir sealed = seule piste qty réaliste.                                                                                                                                                                                                                                                                                                              |
| [AnimeCollection `ids=200`](http://www.animecollection.fr/cartes.php?idl=4&idc=87&ids=200)                                                                                                                      | Ninja Ranks                       | —   | —                             | FR merch                              | pack        | Pack `naruto/ninja-ranks`. Titres EN via checklist officielle Inkworks (100, ledger `inkworks-checklist.json`). Pas de faces. NS européennes hors feuille US.                                                                                                                                                                                                                                                   |
| [AnimeCollection `ids=254`](http://www.animecollection.fr/cartes.php?idl=4&idc=113&ids=254)                                                                                                                     | Panini Ultra                      | —   | —                             | 100 + checklist                       | pack        | Pack `naruto/ultra-challenge` (vide). Éditeur Panini, sept. 2007. **Pas de mini-site produit** sur Wayback (`lamincards.com` parké, Edibas = DBZ, `panini.fr` / `paninicomics.fr` sans chemin Naruto). Journal `narutoultra/curated/sources/wayback-official-hunt.json`.                                                                                                                                        |
| [Collection Naruto YT](https://www.youtube.com/@collection_naruto) `JYwlXQQlooI` + `7r7LwtIENKs` | transcripts staging | — | sealed FR + counts S1–S5 + S6 DVD | Pack Découverte ; displays FR | done | Dig `collection-naruto-youtube-2026-08-29.json`. SKU `pack-decouverte` attested ; `tin-box-hobby` mintée (`contenu-tin-box.jpg`). ASR « Obi » = Hobby. Pas de mint display-s1…s5 sans packshot. |
| [Naruto Collection France FB](https://www.facebook.com/groups/805799330361374) | posts Victor Husson | — | S6 DVD promo MIJ | NI-236 vol. 15 ; communes Shippuden | done | Dig `facebook-naruto-collection-france-2026-09-02.json`. Permalink Jordan 1248487462759223. Pas de mint DVD sans EAN + table cartes. |
...
| [narutocarddass.fandom](https://narutocarddass.fandom.com/wiki/Naruto_CCG_wiki)                                           | —                           | —                             | —                     | PTHN-001 piège          | skip               |                                                                                                                                                                                                           |
...
| [narutocarddass.forumpolish.com t97](https://narutocarddass.forumpolish.com/t97-tradecardonline)                          | —                           | —                             | —                     | mention TCO             | harvest            |                                                                                                                                                                                                           |
...
| [r/Narutoccg](https://www.reddit.com/r/Narutoccg/)                                                              | sheets + EU dig                 | Arctic Shift                  | —                     | CCG N/J/M + Amigo EU    | **harvest** sheets / title attests | Ne pas crawler. Dig 2026-08-26 : `reddit-narutocarddass.json` (Amigo DE, PR-095/096 DE, Europe foil). Accès live JSON 403 → archive API.   |
...
| AnimeCollection Ninja Ranks (`ids=200`) / Ultra card (`ids=254`)                                                                                           | Pas Carddass ; packs `naruto/ninja-ranks` (titres Inkworks, pas de faces) et `naruto/ultra-challenge` (vide)    |
...
- [ ] **4 dossiers de staging qu'aucun code ne lit** — recompté le 2026-08-21, la
  ...
  | ----------------- | ----- | ---------------------------------------------------------- |
  | `narutocarddassfrance` | 1.5 M | fils de forum + un `promo-attested.json` produit sur place |
...
- [x] **Part française `s24` Sage's Legacy** — attestée en 2015 (AnimeCollection)
      comme l'une des deux parts tardives dos noir. Coleka `_r15466` : 121
      annoncées, **120** refs NI/JU/MI uniques (comme le catalogue EN). Faces
      `art.coleka` ~1080×1500 + titres FR. Le starter « La Tempête Approche »
      (33 cartes) reste absent du scellé.
...
- [x] **Reddit r/Narutoccg** — dig 2026-08-26 via Arctic Shift (live JSON 403).
      Ledger `reddit-narutocarddass.json` : Amigo DE checklist ask (`1ucmepn`),
      PR-095 DE + Europe Foil (`1vk1j0c`, `1v67qcf`, `1uthhv7`), PR-096 DE foil
      (`ugdgqq`), storm 3 FR blisters. **Ne pas crawler** le sub ; ne pas
      confondre Amigo Carddass EU et promos UNS3 US-layout.
