...
| `GENERIC_TITLE_TOKENS`, `RESOLVER_GENERIC_TOKENS`, `RESOLVER_PLATFORM_TOKENS`, `NON_CANONICAL_CONTEXT_TOKENS` | `evidence/*`, `identityNoise.ts` | **DRY 2026-07-24**: PriceCharting uses `IDENTITY_FUNCTION_WORDS`; retailer `titleMatch` uses `IDENTITY_EDITION_PACKAGING_TOKENS` + `IDENTITY_PLATFORM_NOISE_TOKENS` (hardware keep-platform). **IDF MVP + offline RawName DF + boot/cron 2026-07-24**. **SAFE micro-shrink 2026-07-24**: `edition`/`version` derived from IDENTITY sets (cold path iso); residual `jeu/game/jeux/games`. Reste : fold media chrome when warm IDF is normal. |
