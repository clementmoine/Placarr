...
| `GENERIC_TITLE_TOKENS`, `RESOLVER_GENERIC_TOKENS`, `RESOLVER_PLATFORM_TOKENS`, `NON_CANONICAL_CONTEXT_TOKENS` | `evidence/*`, `identityNoise.ts` | **DRY 2026-07-24**: PriceCharting uses `IDENTITY_FUNCTION_WORDS`; retailer `titleMatch` uses `IDENTITY_EDITION_PACKAGING_TOKENS` + `IDENTITY_PLATFORM_NOISE_TOKENS` (hardware keep-platform). **IDF MVP**: `tokenCorpusIdf.ts` + optional `CorpusTokenStats` on `evidence/matchUtils.titleSpecificityTokens`; resolve/match paths build in-memory stats from titles already in hand. Full RawName corpus refresh = later P2. |
