...
| `TITLE_PHRASE_EQUIVALENT_GROUPS`, `TITLE_TOKEN_EQUIVALENT_GROUPS` **PARTIEL** (revu 2026-07-04)                                    | `title/tokenEquivalents.ts`               | `TITLE_TOKEN_EQUIVALENT_GROUPS` est bien au niveau dictionnaire (couleurs, `legende/legend`, `criquet/cricket`) ; les variantes d'orthographe sont structurelles (`normalizeEquivalentToken`). **MAIS `TITLE_PHRASE_EQUIVALENT_GROUPS` contient encore 2 sous-titres produit** (`birth of a new world` = AC III, `the american saga` = Star Wars) load-bearing dans `searchVariants.test.ts` — la ligne « FIXED 2026-07-02 » était inexacte. Retrait bloqué sur confirmation que les `regionalTitles`/aliases des providers couvrent ces produits (sinon régression cross-langue). Alignement cross-langue idéal = data-driven via `metadataTitleMatchScore`. |
