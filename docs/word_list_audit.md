...
| **RESIDUAL 2026-07-02**: accent `+50` / French `+30` / `priority>=1 && FR → +1500`                                                | `title/displayScore.ts`                   | Partially executed: word-boosts + CJK gone, the FR/accent boosts remain. Language preference must come from the **UI locale** (`languageOrderForUiLocale`), not a hardcoded FR bias. Consumed by both barcode evidence and metadata merge — test-gated removal + `BARCODE_CACHE_VERSION` bump (displayed titles can change).                                                |
| `TITLE_PHRASE_EQUIVALENT_GROUPS`, `TITLE_TOKEN_EQUIVALENT_GROUPS` (**found 2026-07-02, was un-inventoried**)                      | `title/tokenEquivalents.ts`               | **Per-product FR↔EN subtitle map** ("le roi des corrompus"→"the taken king", "chevaliers de baphomet"→"broken sword", "pit"/"pitt"…) — never exhaustive, product-specific. Target: derive title equivalence from provider **alternate names** (IGDB/ScreenScraper localized titles, `alternateNames.ts`) + consensus; keep only the generic colour/language pairs as data. |
...
| List                                                 | File                        | Verdict                                                                            |
| ---------------------------------------------------- | --------------------------- | ---------------------------------------------------------------------------------- |
| `REAL_BOX_COVER_SOURCES` (`bgg`, `screenscraper`, …) | `attachmentDisplayScore.ts` | Replace with the provider's declared `isRealBoxCover` _capability_, not a name set |
