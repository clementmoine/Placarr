...
| ~~`TITLE_PHRASE_EQUIVALENT_GROUPS`, `TITLE_TOKEN_EQUIVALENT_GROUPS`~~ **FIXED 2026-07-02**                                        | `title/tokenEquivalents.ts`               | **Done**: per-product entries deleted; lists now dictionary-level only (colours, `legende/legend`, `criquet/cricket`, `movie video game`). Spelling variants are structural (`normalizeEquivalentToken`: accents + doubled tail consonants). Cross-language alignment is data-driven: `metadataTitleMatchScore` credits every provider-declared name (title + aliases + regionalTitles). `BARCODE_CACHE_VERSION` v41. |
