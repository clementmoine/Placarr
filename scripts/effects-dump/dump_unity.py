...
3. **textures** — PNG (+ best-effort raw ASTC) under ``public/foil/<pack>/textures/``.

Also exports pack card backs (e.g. ``card_back`` sprite) to ``public/foil/<pack>/card_back.png``.
...
def dump_textures(
    ...
    for obj in env.objects:
        ...
        if img is None:
            ...
            continue
        png_path = textures_dir / f"{tex.m_Name.lower()}.png"
        img.save(png_path)
        ...
        if is_astc_texture(tex):
            ...
            if raw:
                ...
                print(
                    ...
                    f"sans dump brut — PNG seulement"
                )
        print(
            f"  {tex.m_Name} {img.size} wrap={settings['wrap']} "
            f"mips={settings['mipmaps']} -> {png_path.name}{astc_note}"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--pack",
        default="lorcana",
        help="Pack id (default: lorcana) — sorties sous public/foil/<pack>/",
    )
    parser.add_argument(
        "--data",
        required=True,
