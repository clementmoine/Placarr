 ...
 *
 *   tsx scripts/lorcana/cli.ts
 *   tsx scripts/lorcana/cli.ts --providers lorcanaweb lorcanacards
 */
...
function packPython(repo: string): string {
  const venv = path.join(repo, "scripts/lorcana/.venv/bin/python");
...
function runUnity(
  ...
  const args = [
    path.join(repo, "scripts/lorcana/mobile.py"),
    "--repo",
    repo,
  ];
  ...
  const r = spawnSync(packPython(repo), args, {
    ...
    env: {
      ...process.env,
      PYTHONPATH: path.join(repo, "scripts/lorcana"),
    },
  });
