...
function filterPlatformRedundancies(suggestions: string[]): string[] {
...
function isListingDiscardable(title: string): boolean {
...
function versionProvider(provider: string): string {
  return provider.includes(BARCODE_CACHE_VERSION)
    ? provider
    : `${provider}+${BARCODE_CACHE_VERSION}`;
}

function areLikelySameProduct(a: string, b: string): boolean {
