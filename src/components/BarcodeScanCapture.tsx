...
export function BarcodeScanCapture({
  ...
  // Les bascules synchrones (montage caméra / pause) sont ajustées pendant le
  // render ; seul le démontage différé (timer) reste dans l'effect.
  const [prevShowCamera, setPrevShowCamera] = useState(showCamera);
  if (prevShowCamera !== showCamera) {
    setPrevShowCamera(showCamera);
    if (showCamera) {
  ...
  useEffect(() => {
    if (showCamera || !renderCamera) return;
        ...
        {renderCamera ? (
          ...
          >
            <BarcodeScannerView
              ...
              paused={cameraPaused || !showCamera}
            />
