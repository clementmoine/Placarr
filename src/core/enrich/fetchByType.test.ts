...
vi.mock("@/providers/igdb", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/providers/igdb")>();
  return {
    ...actual,
...
vi.mock("@/providers/launchbox", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/providers/launchbox")>();
  return {
    ...actual,
...
vi.mock("@/providers/achatmoinscher", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@/providers/achatmoinscher")>();
  return {
    ...actual,
    fetchFromAchatMoinsCher: h.scraperFetch,
  };
});
vi.mock("@/core/identify/alternateNames", () => ({
  loadBarcodeAlternateNames: vi.fn().mockResolvedValue([]),
}));
vi.mock("@/core/enrich/titleMatching", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@/core/enrich/titleMatching")>();
  return {
    ...actual,
