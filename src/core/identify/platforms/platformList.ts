/**
 * Canonical video-game platform list — **client-safe** (no ScreenScraper /
 * LaunchBox JSON snapshots). Import from here for key membership / labels;
 * `@/core/identify/platforms/platforms` adds source-index maps on top.
 */
...
export const VIDEO_GAME_PLATFORMS = [
  ...
  },
  {
    ...
    aliases: ["nintendo switch 2", "switch 2", "switch2"],
    launchBoxNames: ["Nintendo Switch 2"],
    screenScraperSystemId: 296,
  },
  {
    key: "gamecube",
    label: "GameCube",
...
/** Canonical key from a free-text platform / shelf / title fragment. */
...
export const VIDEO_GAME_PLATFORM_TERMS = Array.from(
  new Set(
    VIDEO_GAME_PLATFORMS.flatMap((platform) =>
...
>();
const THEGAMESDB_PLATFORM_TO_PLATFORM_KEY = new Map<
  number,
  VideoGamePlatformKey
>();
for (const platform of VIDEO_GAME_PLATFORM_DEFINITIONS) {
  if (platform.theGamesDbId) {
    THEGAMESDB_PLATFORM_TO_PLATFORM_KEY.set(
      platform.theGamesDbId,
      platform.key,
    );
  }
...
export function getPlatformKeyByTheGamesDbPlatformId(
  platformId?: number | null,
): VideoGamePlatformKey | null {
  if (!platformId) return null;
  return THEGAMESDB_PLATFORM_TO_PLATFORM_KEY.get(platformId) ?? null;
}
