...
vi.mock("@/core/metadata/selection", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@/core/metadata/selection")>();
  return {
    ...actual,
...
vi.mock("@/providers/igdb", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@/providers/igdb")>();
  return {
    ...actual,
    fetchFromIGDB: h.fetchFromIGDB,
  };
});
vi.mock("@/providers/howlongtobeat", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@/providers/howlongtobeat")>();
  return {
    ...actual,
    fetchFromHowLongToBeat: h.fetchFromHowLongToBeat,
  };
});
vi.mock("@/providers/steamgriddb", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@/providers/steamgriddb")>();
  return {
    ...actual,
...
vi.mock("@/providers/coverproject", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@/providers/coverproject")>();
  return {
    ...actual,
    fetchFromCoverProject: h.fetchFromCoverProject,
  };
});
vi.mock("@/providers/launchbox", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@/providers/launchbox")>();
  return {
    ...actual,
    fetchFromLaunchBox: h.fetchFromLaunchBox,
  };
});
vi.mock("@/providers/thegamesdb", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@/providers/thegamesdb")>();
  return {
    ...actual,
    fetchFromTheGamesDB: h.fetchFromTheGamesDB,
  };
});
vi.mock("@/providers/achatmoinscher", async (importOriginal) => {
  const actual =
    await importOriginal<
      typeof import("@/providers/achatmoinscher")
    >();
  return {
    ...actual,
...
vi.mock("@/core/barcode/alternateNames", () => ({
  loadBarcodeAlternateNames: vi.fn().mockResolvedValue([]),
}));
vi.mock("@/core/metadata/titleMatching", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@/core/metadata/titleMatching")>();
  return {
    ...actual,
...
} from "@/core/metadata/fetchByType";
