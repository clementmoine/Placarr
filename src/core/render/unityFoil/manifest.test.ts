...
import {
  UNITY_FOIL_MATERIAL_NAMES,
  secondTopLayerFragment,
  unityFoilMaterial,
  unityFoilMaterialForPrint,
} from "./manifest";

/**
 * The manifest against the artefacts it names, and both against the shaders.
 *
 * The whole point of the Unity port is that *nothing* is hand-assembled: the
 * manifest is generated from the app's bundle, the fragments are the app's
 * own programs, and the renderer binds them by reflection. What can still rot
 * is the seam between them — a regenerated manifest naming a fragment that was
 * not re-extracted, a texture file renamed on disk, a sampler the shader
 * declares that no binding fills (which draws as a black layer, the exact
 * "layer silently missing" failure this port exists to end).
 */
...
/**
 * Every `_Name` uniform a fragment declares, dead weight excluded — the same
 * reading the generator does, re-derived here so a generator bug cannot
 * vouch for itself.
 */
...
describe("manifest unityFoil", () => {
  it("couvre les 13 finitions du vocabulaire de l'app", () => {
    // Chaque finition doit avoir au moins un matériau — une finition sans
    // matériau retomberait silencieusement sur le rendu CSS.
    const corpus = UNITY_FOIL_MATERIAL_NAMES.join(" ");
    for (const finish of [
      "Silver",
      "Satin",
      "Lore",
  ...
  it("expose un fragment Time pour chaque matériau (idle natif)", () => {
    for (const name of UNITY_FOIL_MATERIAL_NAMES) {
      const material = unityFoilMaterial(name)!;
  ...
  for (const name of UNITY_FOIL_MATERIAL_NAMES) {
    ...
    describe(name, () => {
      ...
      it("nomme un fragment qui existe et parle WebGL2", () => {
        ...
        // Les deux macros doivent être à 0, sinon le source garde des blocs
        // UBO et des layout(binding) que WebGL2 refuse de compiler.
      ...
      it("remplit chaque sampler que ses fragments déclarent", () => {
        // Un sampler sans binding rend une couche noire, en silence — c'est
        // le « layer oublié » que tout ce port existe à éliminer.
      ...
      it("lie des textures présentes sur disque, ou des rôles connus", () => {
        for (const binding of Object.values(material.textures)) {
          if (binding.file) {
            ...
            // Sampler state comes from the APK Texture2D — not invented here.
      ...
      it("ne porte que des valeurs que l'un des fragments déclare", () => {
        // Le manifest est filtré par fragment à la génération ; une valeur
        // orpheline signale un manifest et des shaders régénérés séparément.
        ...
        for (const source of [
          ...
          for (const name of fragmentUniforms(source)) declared.add(name);
        }
        const orphans = [
...
describe("USESECONDTOPLAYER", () => {
  it("bascule MagmaMetallicHotFoil vers le sibling qui sample le 2e masque", () => {
    // Deux prints du catalogue FR portent un second stamp ; sans ce basculement
    // le masque est passé au renderer mais le fragment ne le lit jamais.
    const upgraded = unityFoilMaterialForPrint("CardMagmaMetallicHotFoil", {
  ...
  it("ne change rien sans second masque", () => {
    const base = unityFoilMaterial("CardMagmaMetallicHotFoil")!;
    const same = unityFoilMaterialForPrint("CardMagmaMetallicHotFoil", {});
  ...
  it("ne invente pas de sibling pour Silver (pas de variante HotFoil)", () => {
    ...
    const same = unityFoilMaterialForPrint("CardFoilSilver", {
      secondVarnishMaskUrl: "/uploads/second.png",
    });
