...
function sharedTitlePrefix(left: string, right: string): string {
  const leftTokens = normalizeVolumeTitleText(left).split(/\s+/).filter(Boolean);
  const rightTokens = normalizeVolumeTitleText(right).split(/\s+/).filter(Boolean);
  const shared: string[] = [];
...
function distinctEditionAfterSharedPrefix(
  ...
  if (
    !sharedPrefix ||
    normalizeVolumeTitleText(itemTitle) === normalizeVolumeTitleText(catalogTitle)
  ) {
    return false;
  }
