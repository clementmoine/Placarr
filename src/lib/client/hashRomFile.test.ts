...
/** Node 26 types `Uint8Array` as `ArrayBufferLike`; `Blob` wants `ArrayBuffer`. */
...
describe("hashRomFile", () => {
  ...
  it("streams Blob slices with tiny chunks and matches one-shot digests", async () => {
    ...
    const blob = new Blob([
      blobPart(bytes.subarray(0, 7)),
      blobPart(bytes.subarray(7, 40)),
      blobPart(bytes.subarray(40)),
    ]);
  ...
  it("reports progress from 0 to complete while streaming", async () => {
    ...
    const ratios: number[] = [];
    await hashRomFile(new Blob([blobPart(bytes)]), {
      chunkBytes: 16,
      onProgress: (progress) => {
