...
// Confidence of a consensus value grows with the number of independent sources
// that agree: a base for a single source, plus a fixed increment per extra
// agreeing source, capped at 1. One formula for every consensus field, so the
// model lives in one place instead of as a repeated `0.5 + n * 0.15` literal.
const CONSENSUS_BASE_CONFIDENCE = 0.5;
const CONSENSUS_PER_SOURCE_CONFIDENCE = 0.15;
function agreementConfidence(agreeingSources: number): number {
  return clamp01(
    CONSENSUS_BASE_CONFIDENCE +
      agreeingSources * CONSENSUS_PER_SOURCE_CONFIDENCE,
  );
}
...
export function computeRatingConsensus(
  ...
  return {
    ...
    confidence: agreementConfidence(parsed.length),
    priority: 200,
  };
}
...
export function computeAgeConsensus(
  ...
  return {
    ...
    confidence: agreementConfidence(ages.length),
    priority: 150,
  };
}
