...
function usageForTitleRole(role: TitleObservationRole): MetadataObservationUsage {
  switch (role) {
    case "object_title":
    case "catalog_title":
...
function usageForImageRole(role: ImageObservationRole): MetadataObservationUsage {
  switch (role) {
    case "cover_front":
    case "cover_back":
...
function usageForAliasRole(role: AliasObservationRole): MetadataObservationUsage {
  switch (role) {
  ...
  for (const fact of metadata.facts || []) {
    observations.push({
      ...
      usage: makeObservationUsage({
        evidence:
          options.factRole === "structured_fact" ? "strong" : "normal",
      }),
    });
  }
