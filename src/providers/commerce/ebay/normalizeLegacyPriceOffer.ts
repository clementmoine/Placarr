import { getEbayMarketplaceId } from "./ebayConfig";

/** Legacy price rows stored before the PicClick provider was removed (2026-07). */
const LEGACY_PICCLICK_SOURCE = /^picclick$/i;

/** Display label shared with live eBay price offers (`index.ts` PRICE_SOURCE). */
const EBAY_PRICE_SOURCE_LABEL = "eBay";
...
export function normalizeLegacyPriceOffer<T extends NormalizablePriceOffer>(
  ...
  return {
    ...offer,
    source: EBAY_PRICE_SOURCE_LABEL,
    sourceUrl: ebayUrl ?? offer.sourceUrl,
    merchantName: offer.merchantName?.trim()
      ? offer.merchantName
      : EBAY_PRICE_SOURCE_LABEL,
    rawValue,
  };
}
