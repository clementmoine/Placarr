/**
 * Install Chitoroshop JA faces (+ optional EN titles) onto dbsjcc prints.
 *
 * Matching: prefer title/body setHint; else lowest partN over promo/sp.
 * JA-only cards (e.g. D-584) mint a new print when setHint is known.
...
export type InstallChitoroshopJaReport = {
  listed: number;
  matched: number;
  minted: number;
  titles: number;
...
export async function installDbsJccChitoroshopJa(
  index: LocalPrintsIndex,
  opts: { downloadFaces?: boolean } = {},
): Promise<InstallChitoroshopJaReport> {
  ...
  for (const row of faces) {
    ...
    const decision = matchChitoroshopToDbsjccPrint(
      row.number,
      row.setHint,
      candidates,
    );
    ...
    if (decision.kind === "skip") {
      ...
      console.log(`── chitoroshop skip — ${row.printed}: ${decision.reason}`);
      continue;
    }
    ...
    // Mint always needs a prints row; match only when we have an EN title to add
    ...
    const artName = "art.chitoroshop.jpg";
