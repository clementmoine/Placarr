...
import { isColekaPlaceholderName, fillNarutoTitlesFromSiblingLocales, mergeBrasilUsExclusivesIntoIndex, mergeChecklistFrNamesIntoIndex, mergeColekaFrNamesIntoIndex, mergeCardgameclubItRarities, mergeFoundCatalogueLedgers, mergeHinokunianJaNamesIntoIndex, copyNarutoTitlesOntoGroupedPrints, mergeTitleCorrections, mergeSlabZJaNamesIntoIndex, mergeUserPhysicalCcgIntoIndex } from "./ledgers";

// —— mergeAttestedLedgers ——
{
  ...
  describe("mergeUserPhysicalCcgIntoIndex", () => {
    it("mints UNS3 EU promos and does not invent a second Kisame key", () => {
      ...
      expect(
        merged.titles.find((t) => t.printKey === "naruto:pr-0095"),
      ).toMatchObject({ lang: "fr", fullName: "Naruto Uzumaki" });
      expect(
        merged.titles.find((t) => t.printKey === "naruto:pr-0096"),
      ).toMatchObject({ lang: "fr", fullName: "4E Hokage" });
      expect(
        merged.titles.find((t) => t.printKey === "naruto:pr-0100"),
      ).toMatchObject({ lang: "fr", fullName: "Naruto Uzumaki" });
      expect(
