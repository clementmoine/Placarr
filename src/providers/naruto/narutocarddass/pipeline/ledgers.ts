/**
 * Action module: pipeline/ledgers.ts
 * Merged from: mergeAttestedLedgers.ts
 */
...
import { isColekaPlaceholderName } from "./index";
...
import { mergeGgClassicTitlesIntoIndex } from "../scrape/catalogues";
import {
  mergeNarutoCardsCaIntoIndex,
  type NarutoCardsCaCard,
} from "../parse/catalogues";
import { mergeNarutoCardsCaLedgerIntoIndex } from "../scrape/catalogues";
import { mergeNarutoCardsNetLedgerIntoIndex } from "../scrape/catalogues";
import { mergeCollectorsCometLedgerIntoIndex } from "../scrape/catalogues";
import { mergeFansetEnTitlesIntoIndex } from "../parse/catalogues";
import { loadColekaCarddassFrLedger } from "../scrape/coleka";
import { mergeTvTokyoJaNamesIntoIndex } from "../sources/faces";


// --- from mergeAttestedLedgers.ts ---
...
export { isColekaPlaceholderName } from "./index";
...
function printGrouping(print: NarutoPrintRow): string | null {
  ...
  const fromNumber = parseNarutoCollector(print.number)?.grouping;
  if (fromNumber?.trim()) return fromNumber.trim().toLowerCase();
  /*
    Disk rows for tourney stamps often keep `number: nus0005` (folder id) while
    the printKey alone carries `-promo`. Without reading the key, copy titles
    treats retail + promo as two bases and leaves the stamp nameless.
  */
  const fromKey = parsePrintKey(print.printKey)?.grouping;
  return fromKey?.trim().toLowerCase() || null;
}
...
/**
 * In-hand CCG copies. UNS3 EU wave: PR-095 / PR-096 / PR-100 FR titles.
...
export function mergeFoundCatalogueLedgers(input: {
  ...
  // IT volontairement hors assemblage catalogue (contrat JA+FR+EN).
  const physicalCcg = mergeUserPhysicalCcgIntoIndex(brasilUs);
  ...
  const fixed = mergeTitleCorrections(grouped);
  return {
    prints: fixed.prints,
    titles: fixed.titles,
    ...
    tvtokyoAdded: tvtokyo.addedPrints,
    tvtokyoTitled: tvtokyo.titled,
