/**
 * Action module: sealed.ts
 * Merged from: volumeOfficialProducts.ts, tinBoxPromos.ts, sealedProducts.ts
 */
...
import { germanSealedReleases } from "./sources/sealed";
import { japaneseSealedReleases } from "./sources/sealed";
import { narutoCuratedDir, narutoCuratedProductsDir } from "./install/curated";
import {
...
import starterBoxAccessoriesJson from "./curated/sources/starter-box-accessories.json";


// --- from volumeOfficialProducts.ts ---
...
// ─── starter box accessories ────────────────────────────────────


export function starterBoxAccessoriesLedger() {
  return starterBoxAccessoriesJson;
}

export function starterBoxAccessories(setCode: string) {
  const code = setCode.toLowerCase();
  if (!(starterBoxAccessoriesJson.sku.attestedSets as readonly string[]).includes(code)) {
    return null;
  }
  return {
    cardCount: starterBoxAccessoriesJson.sku.cardCount,
    livretDeJeu: starterBoxAccessoriesJson.sku.livretDeJeu,
    plateauDeJeu: starterBoxAccessoriesJson.sku.plateauDeJeu,
    jetons: starterBoxAccessoriesJson.sku.jetons,
    jetonNarutoAmongTokens: starterBoxAccessoriesJson.sku.jetonNarutoAmongTokens,
    livretCartesACollectionner: starterBoxAccessoriesJson.sku.livretCartesACollectionner,
  };
}
