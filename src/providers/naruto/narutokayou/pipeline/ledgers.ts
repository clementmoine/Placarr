...
export { KAYOU_TITLE_LANG };
import {
  buildKayouOfficialChecklist,
...
import {
  ...
  readNarutodbChecklist,
  readNarutopiaKayouImageIndex,
} from "../sources/crawl";
import { enrichChecklistWithOfficialFaces } from "../sources/faces";
import { mergeKayouChecklists } from "./checklists";

export type { KayouChecklistCard, KayouChecklistSet } from "../identity";
...
export function readKayouChecklist(): KayouChecklist {
  ...
  const official = buildKayouOfficialChecklist(readKayouOfficialCatalog());
  if (official) extras.push({ ledger: official, source: "kayouofficial" });
  const narutodb = readNarutodbChecklist();
  if (narutodb) extras.push({ ledger: narutodb, source: "narutodb" });
  let merged =
    extras.length > 0
  ...
  return enrichChecklistWithOfficialFaces(merged, readKayouOfficialCatalog());
}
