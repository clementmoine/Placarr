...
describe("productSourceFromStagingRel", () => {
  it("maps each packshot host to a dump source", () => {
    ...
    expect(
      productSourceFromStagingRel("staging/leboncoin/tin-box-01.jpg"),
    ).toBe("leboncoin");
    expect(
      productSourceFromStagingRel(
    ...
    expect(
      productSourceFromStagingRel("staging/kinkai/duopack-s28-02.webp"),
    ).toBe("kinkai");
    expect(
      productSourceFromStagingRel("staging/sunnystore/booster-s28-01.jpg"),
    ).toBe("sunnystore");
    expect(
      productSourceFromStagingRel(
    ...
    expect(
      productSourceFromStagingRel(
        "staging/manga-sanctuary/starter-la-quete.jpg",
      ),
    ).toBe("manga-sanctuary");
    expect(
      productSourceFromStagingRel(
        "staging/bandai-fr-us-drive/packshots-by-sku/displays/s13.png",
      ),
    ).toBe("bandai-drive");
    expect(
...
describe("pickBestNarutoProductDump", () => {
  it("falls back to locale priority when dumps have no pixels", () => {
    ...
    expect(NARUTO_PRODUCT_PRIORITY.fr).toContain("manga-sanctuary");
    expect(NARUTO_PRODUCT_PRIORITY.en).toContain("bandai-drive");
    expect(NARUTO_PRODUCT_PRIORITY.es[0]).toBe("badge");
    expect(NARUTO_PRODUCT_PRIORITY.es[1]).toBe("sunnystore");
  });
});
