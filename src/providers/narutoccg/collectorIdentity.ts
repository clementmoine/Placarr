...
export function compareNarutoLangs(a: string, b: string): number {
  const ia = LANG_ORDER.indexOf(a.toLowerCase() as (typeof LANG_ORDER)[number]);
  const ib = LANG_ORDER.indexOf(b.toLowerCase() as (typeof LANG_ORDER)[number]);
