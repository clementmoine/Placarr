...
describe("EN CCG source ledgers", () => {
  ...
  it("keeps r/Narutoccg thread 1vjw5hb as rules, not a face dump", () => {
    expect(redditNarutoccg.ingest).toBe("none");
    expect(redditNarutoccg.crawl).toBe(false);
    expect(redditNarutoccg.line).toBe("en-ccg");
    const thread = redditNarutoccg.threads.find((row) => row.id === "1vjw5hb");
    expect(thread?.kind).toBe("rules");
    expect(thread?.cited).toMatchObject({
      printed: "N-1080",
      diskHint: "n1080",
    });
    expect(thread?.ingestPhoto).toBe(false);
    expect(redditNarutoccg.stillWanted).toContain("inventory-sheets");
  });
  ...
  it("keeps the collector's French CCG copies (PR-096 ≠ s28 booster)", () => {
  ...
  it("indexes every URL pasted in chat today", () => {
    ...
    expect(trictrac.products.every((row) => row.kind === "deck")).toBe(true);
    ...
    expect(urls).toContain("https://narutoccg.fandom.com/wiki/Naruto_CCG_wiki");
