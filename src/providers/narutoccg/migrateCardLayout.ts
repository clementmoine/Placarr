...
export function applyNarutoLayoutMoves(
  ...
  const appearances: Record<string, Record<string, string>> = {};
  let moved = 0;
  for (const move of moves) {
    ...
    langs[move.lang] = move.appearanceSet;
...
export function upsertNarutoAppearances(
  ...
  let appearances: Record<string, Record<string, string>> = {};
  if (existsSync(file)) {
    try {
  ...
  for (const row of rows) {
    ...
    langs[row.lang] = row.appearanceSet;
...
export function migrateNarutoCardLayout(opts?: {
  ...
  let enMoved = 0;
  const enAppearances: Record<string, Record<string, string>> = {
  ...
  const mergedAppearances = { ...previous };
  for (const [diskId, langs] of Object.entries(enAppearances)) {
    mergedAppearances[diskId] = { ...mergedAppearances[diskId], ...langs };
  }
  writeFileSync(
    appearancesPath,
    `${JSON.stringify(
