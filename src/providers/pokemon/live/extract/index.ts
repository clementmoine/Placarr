...
import {
  ...
  type BundleResult,
  type DirProbeMode,
} from "@/providers/pokemon/live/cdn";
import {
  bootstrapMalieCatalogue,
  filterMalieStemsByLangs,
  loadMalieBundleStems,
} from "@/providers/pokemon/live/cdn/malie";
import { POKEMON_LIVE_SCRAPE_DEFAULT_LANGUAGES } from "@/providers/pokemon/live/cdn/languages";
import {
  buildScrapeInventory,
  mergeCdnResultsIntoInventory,
  mergeLiveIdentities,
} from "@/providers/pokemon/live/cdn/scrapeInventory";
import { loadCdnCatalogue } from "@/providers/pokemon/live/cdn/scrapePlan";
...
} from "@/providers/pokemon/live/cdn/cardDatabase";
import {
  resolveCdnTarget,
  writeSourcesReport,
...
async function runManifestDump(
  ...
  const { directoriesFromDirsManifest } = await import(
    "@/providers/pokemon/live/cdn/dumpCdnManifest"
  );
...
async function runExtract(
  ...
  const { extractAllNode } = await import(
    "@/providers/pokemon/live/extract/extractAllNode"
  );
...
export async function runUpdate(
  ...
  if (opts.bootstrapMalie !== false) {
    ...
    const { yieldMalieEventLoop } = await import(
      "@/providers/pokemon/live/cdn/malie"
    );
  ...
  if (!opts.skipStoreAudit) {
    ...
    const storeCmd = [
      "exec",
      "tsx",
      "src/providers/pokemon/live/extract/audit_store.ts",
    ];
  ...
  if (opts.skipPaperFaces) {
    ...
    try {
      const { runPokemonPaperFacesHarvest } = await import(
        "@/providers/pokemon/tcgdex/scrape/paperFacesHarvest"
      );
