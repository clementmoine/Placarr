"""Pack paths for the Unity island — re-exports ``unity/lib/paths.py``.

Loaded via ``importlib`` so ``PYTHONPATH=…/unity`` cannot recurse into this
shim when resolving ``paths``. Prefer putting ``unity/lib`` first on
``PYTHONPATH`` so ``import paths`` hits lib directly.
"""
...
_LIB_PATHS = Path(__file__).resolve().parent / "lib" / "paths.py"
