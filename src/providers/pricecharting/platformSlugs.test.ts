...
describe("priceCharting platform slugs", () => {
  ...
  it("maps modern consoles including Xbox One (FIFA 17 direct URLs)", () => {
    ...
    expect(
      resolvePriceChartingPlatformSlug("Xbox One", { isPal: true }),
    ).toBe("pal-xbox-one");
    expect(resolvePriceChartingPlatformSlug("Xbox One")).toBe("xbox-one");
    expect(getPriceChartingPlatformSlugs("ps4")?.default).toBe(
      "playstation-4",
    );
  ...
  it("maps handheld / Sega / Atari shelves used in Placarr", () => {
    expect(resolvePriceChartingPlatformSlug("Game Boy Advance", { isPal: true })).toBe(
      "pal-gameboy-advance",
    );
    expect(resolvePriceChartingPlatformSlug("Mega Drive", { isPal: true })).toBe(
      "pal-sega-mega-drive",
    );
    ...
    expect(resolvePriceChartingPlatformSlug("Atari 2600", { isPal: true })).toBe(
      "pal-atari-2600",
    );
    expect(resolvePriceChartingPlatformSlug("Nintendo Switch 2", { isPal: true })).toBe(
      "pal-nintendo-switch-2",
    );
