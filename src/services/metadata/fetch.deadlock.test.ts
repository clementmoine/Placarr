...
import { resetMetadataProviderQueuesForTests } from "@/lib/metadata/providerQueue";
...
vi.mock("@/services/provider/bootstrap", async () => {
  const { runQueuedMetadataProviderCall } = await vi.importActual<
    typeof import("@/lib/metadata/providerQueue")
  >("@/lib/metadata/providerQueue");
  return {
  ...
  it("does not deadlock when missing providers fall through to the fallback pass", async () => {
    ...
    expect(
      mockResolve.mock.calls.some(
        ([ctx, id]) =>
          id !== anchorId &&
          (ctx as { name: string }).name === "Bilbo Le Hobbit",
      ),
    ).toBe(true);
  });
});
